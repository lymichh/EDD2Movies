...
class AVLTree:
    ...
    def search_condiciones_string(self, year, int_earnings):
        list = self.search_condiciones2(self.root, year, int_earnings)
        return list
    
    def search_condiciones2(self, node, year, international_earnings) -> list:
        List = []
        if node is not None:
            List.extend(self.search_condiciones2(node.left, year, international_earnings))
            if (node.data.year == year) and (node.data.domestic_percent < node.data.foreign_percent) and (node.data.foreign_earnings >= international_earnings):
                List.append(node)
            List.extend(self.search_condiciones2(node.right, year, international_earnings))
...
def buscar_metrica_view():
    ...
    def on_buscar_click_metrica():
        ...
        Lista_movies= avl_tree.search_condiciones_string(year_buscar, earnings_buscar)  # Buscar el nodo con la película

        # Habilitar el textbox para actualizar su contenido
        buscar_metrica_text.configure(state="normal")
        buscar_metrica_text.delete("0.0", "end")  # Limpiar el contenido actual

        # Inicializar 'parrafo_recorrido'
        parrafo_recorrido = ""
        if Lista_movies is not None:
            c = 1
            for movie in Lista_movies:
                parrafo_recorrido += f"{c}. Title: {movie.data.title},Year: {movie.data.year}, Worldwide Earnings: {movie.data.worldwide_earnings},Domestic Earnings: {movie.data.domestic_earnings} ({movie.data.domestic_percent_earnings}%), Foreign Earnings: {movie.data.foreign_earnings} ({movie.data.foreign_percent_earnings}%)\n"
                c += 1
        else:
    ...
    # Crear los botones
    nivel_nodo_metrica_btn = CTkButton(master=botones_metrica_frame, text="Nivel nodo", image=nivel_nodo_icon, fg_color="#008CFF", hover_color="#00A3FF", font=("Arial Black", 14), corner_radius=10, text_color="#FFFFFF", height=50, width=160, border_width=2, border_color="#006BBF")
    nivel_nodo_metrica_btn.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

    factor_balanceo_metrica_btn = CTkButton(master=botones_metrica_frame, text="Factor balanceo", image=factor_balanceo_icon, fg_color="#008CFF", hover_color="#00A3FF", font=("Arial Black", 14), corner_radius=10, text_color="#FFFFFF", height=50, width=160, border_width=2, border_color="#006BBF")
    factor_balanceo_metrica_btn.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")

    nodo_padre_metrica_btn = CTkButton(master=botones_metrica_frame, text="Nodo padre", image=nodo_padre_icon, fg_color="#008CFF", hover_color="#00A3FF", font=("Arial Black", 14), corner_radius=10, text_color="#FFFFFF", height=50, width=160, border_width=2, border_color="#006BBF")
    nodo_padre_metrica_btn.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")

    nodo_abuelo_metrica_btn = CTkButton(master=botones_metrica_frame, text="Nodo abuelo", image=nodo_abuelo_icon, fg_color="#008CFF", hover_color="#00A3FF", font=("Arial Black", 14), corner_radius=10, text_color="#FFFFFF", height=50, width=160, border_width=2, border_color="#006BBF")
    nodo_abuelo_metrica_btn.grid(row=1, column=0, padx=10, pady=10, columnspan=2)

    nodo_tio_metrica_btn = CTkButton(master=botones_metrica_frame, text="Nodo tío", image=nodo_tio_icon, fg_color="#008CFF", hover_color="#00A3FF", font=("Arial Black", 14), corner_radius=10, text_color="#FFFFFF", height=50, width=160, border_width=2, border_color="#006BBF")
    ...
    parrafo2_recorrido = """¡Bienvenido al Sistema de Gestión de Películas! Aquí podrás administrar tu colección de películas de manera fácil y rápida. Ya sea que desees agregar nuevas películas, buscar tus favoritas, actualizar información o eliminar títulos, este programa te brinda todas las herramientas necesarias para mantener organizada tu biblioteca de películas. ¡Comencemos!"""
